...
if (is_resource($lock)) {
    ...
    if (is_resource($process)) {
        ...
        assert_true(str_contains($early_body, 'function compactValuePreview'), 'out-of-band branch manager keeps resolution value previews compact');
        assert_true(str_contains($early_body, 'Target branch value'), 'out-of-band branch manager explains target-branch conflict resolutions directly');
        assert_true(str_contains($early_body, 'too long to display'), 'out-of-band branch manager avoids rendering huge resolution values inline');
        assert_true(!str_contains($early_body, 'Current target value'), 'out-of-band branch manager does not use abstract current-target resolution labels');
        ...
        assert_true(str_contains($early_body, 'extensionLabel +'), 'out-of-band branch manager labels plugin/theme metadata disclosures by conflict scope');
        assert_true(str_contains($early_body, 'Conflict check payload'), 'out-of-band branch manager gives plugin/theme payload details a neutral label');
        ...
        assert_true(str_contains($early_body, "disclosure(extensionLabel + ' details', pluginPanel, false)"), 'out-of-band branch manager keeps plugin/theme metadata in collapsed secondary details');
        ...
        assert_true(str_contains($early_body, 'No approved automation for this'), 'out-of-band branch manager labels plugin and theme conflicts without configured drivers clearly');
        ...
        assert_true(!str_contains($early_body, 'function pluginDriverExplanationPanel'), 'out-of-band branch manager does not duplicate missing-driver explanation in a separate panel');

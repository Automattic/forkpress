...
try {
    ...
    assert_same(
        ...
        1,
        'unsafe file-to-directory replacement records a type-specific target-wins decision'
    );
    ...
    assert_same(count($audit['conflicts']), 4, 'file audit can focus on unsafe symlink conflicts');
    ...
    assert_true(in_array(cow_merge_file_identity_json('wp-content/uploads/links/root-escape-link.txt'), $unsafe_symlink_identities, true), 'unsafe symlink audit points at the blocked root-escaping path');
    ...
    $delete_dir_audit = cow_merge_audit_report($metadata, (int)$result['run_id'], 10, [
        'scope' => 'files',
        'records' => 'conflicts',
        'conflict_type' => 'file-directory-delete-conflict',
    ]);
    assert_same(count($delete_dir_audit['conflicts']), 1, 'file audit can focus on directory deletion conflicts');
    assert_same(
    ...
    $unsafe_file_to_dir_conflict_id = (int)scalar($metadata, "SELECT id FROM merge_conflicts WHERE table_name = '__files__' AND conflict_type = 'file-type-replacement-conflict' AND row_identity = '$unsafe_file_to_dir_identity' ORDER BY id DESC LIMIT 1");
    $unsafe_subtree_resolution_error = null;
    try {
        cow_merge_resolve_conflict($metadata, $unsafe_file_to_dir_conflict_id, 'source', true, 'Try applying unsafe source directory subtree.', 'cow-test');
    } catch (Throwable $e) {

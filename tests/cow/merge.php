...
try {
    ...
    $rowid_insert_finalize_base = $tmp . '/rowid-insert-finalize-base.sqlite';
    $rowid_insert_finalize_source = $tmp . '/rowid-insert-finalize-source.sqlite';
    $rowid_insert_finalize_target = $tmp . '/rowid-insert-finalize-target.sqlite';
    $rowid_insert_finalize_metadata = $tmp . '/.forkpress/cow/merge/rowid-insert-finalize-metadata.sqlite';
    create_base_db($rowid_insert_finalize_base);
    copy($rowid_insert_finalize_base, $rowid_insert_finalize_source);
    copy($rowid_insert_finalize_base, $rowid_insert_finalize_target);
    $db = open_db($rowid_insert_finalize_source);
    $db->exec('CREATE TABLE plugin_rowid_mutation_insert (label TEXT, value TEXT)');
    $db->exec("INSERT INTO plugin_rowid_mutation_insert (rowid, label, value) VALUES (19, 'source rowid insert', 'source')");
    $db->close();
    $GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize'] = [
        static function (SQLite3Result $result, string $message): void {
            if ($message === 'failed to finalize rowid insert into plugin_rowid_mutation_insert') {
                throw new RuntimeException('forced rowid insert finalize failure');
            }
        },
    ];
    assert_throws(
        fn() => cow_merge_databases(
            $rowid_insert_finalize_base,
            $rowid_insert_finalize_source,
            $rowid_insert_finalize_target,
            $rowid_insert_finalize_metadata,
            'feature-rowid-insert-finalize-rollback',
            'main'
        ),
        'forced rowid insert finalize failure',
        'source rowid insert finalization failures surface to the caller'
    );
    unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize']);
    assert_same((int)scalar($rowid_insert_finalize_target, "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'plugin_rowid_mutation_insert'"), 0, 'failed source rowid insert finalization rolls back source-added table creation');
    assert_same((int)scalar($rowid_insert_finalize_metadata, "SELECT COUNT(*) FROM merge_runs WHERE source_branch = 'feature-rowid-insert-finalize-rollback' AND status = 'failed'"), 1, 'failed source rowid insert finalization leaves an auditable failed run');
    assert_same((int)scalar($rowid_insert_finalize_metadata, 'SELECT COUNT(*) FROM merge_row_identities'), 0, 'failed source rowid insert finalization records no sidecar metadata');
    assert_same((int)scalar($rowid_insert_finalize_metadata, 'SELECT COUNT(*) FROM merge_decisions'), 0, 'failed source rowid insert finalization records no decisions');
    ...
    $row_update_finalize_base = $tmp . '/row-update-finalize-base.sqlite';
    $row_update_finalize_source = $tmp . '/row-update-finalize-source.sqlite';
    $row_update_finalize_target = $tmp . '/row-update-finalize-target.sqlite';
    $row_update_finalize_metadata = $tmp . '/.forkpress/cow/merge/row-update-finalize-metadata.sqlite';
    copy($row_update_execute_base, $row_update_finalize_base);
    copy($row_update_execute_source, $row_update_finalize_source);
    copy($row_update_execute_target, $row_update_finalize_target);
    $GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize'] = [
        static function (SQLite3Result $result, string $message): void {
            if ($message === 'failed to finalize update on plugin_row_mutation_update') {
                throw new RuntimeException('forced row update finalize failure');
            }
        },
    ];
    assert_throws(
        fn() => cow_merge_databases(
            $row_update_finalize_base,
            $row_update_finalize_source,
            $row_update_finalize_target,
            $row_update_finalize_metadata,
            'feature-row-update-finalize-rollback',
            'main'
        ),
        'forced row update finalize failure',
        'source row update finalization failures surface to the caller'
    );
    unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize']);
    assert_same(scalar($row_update_finalize_target, 'SELECT value FROM plugin_row_mutation_update WHERE id = 1'), 'base update', 'failed source row update finalization rolls back target rows');
    assert_same((int)scalar($row_update_finalize_metadata, "SELECT COUNT(*) FROM merge_runs WHERE source_branch = 'feature-row-update-finalize-rollback' AND status = 'failed'"), 1, 'failed source row update finalization leaves an auditable failed run');
    assert_same((int)scalar($row_update_finalize_metadata, 'SELECT COUNT(*) FROM merge_decisions'), 0, 'failed source row update finalization records no decisions');
    ...
    $row_delete_finalize_base = $tmp . '/row-delete-finalize-base.sqlite';
    $row_delete_finalize_source = $tmp . '/row-delete-finalize-source.sqlite';
    $row_delete_finalize_target = $tmp . '/row-delete-finalize-target.sqlite';
    $row_delete_finalize_metadata = $tmp . '/.forkpress/cow/merge/row-delete-finalize-metadata.sqlite';
    copy($row_delete_execute_base, $row_delete_finalize_base);
    copy($row_delete_execute_source, $row_delete_finalize_source);
    copy($row_delete_execute_target, $row_delete_finalize_target);
    $GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize'] = [
        static function (SQLite3Result $result, string $message): void {
            if ($message === 'failed to finalize delete from plugin_row_mutation_delete') {
                throw new RuntimeException('forced row delete finalize failure');
            }
        },
    ];
    assert_throws(
        fn() => cow_merge_databases(
            $row_delete_finalize_base,
            $row_delete_finalize_source,
            $row_delete_finalize_target,
            $row_delete_finalize_metadata,
            'feature-row-delete-finalize-rollback',
            'main'
        ),
        'forced row delete finalize failure',
        'source row delete finalization failures surface to the caller'
    );
    unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize']);
    assert_same((int)scalar($row_delete_finalize_target, 'SELECT COUNT(*) FROM plugin_row_mutation_delete WHERE id = 1'), 1, 'failed source row delete finalization rolls back target rows');
    assert_same((int)scalar($row_delete_finalize_metadata, "SELECT COUNT(*) FROM merge_runs WHERE source_branch = 'feature-row-delete-finalize-rollback' AND status = 'failed'"), 1, 'failed source row delete finalization leaves an auditable failed run');
    assert_same((int)scalar($row_delete_finalize_metadata, 'SELECT COUNT(*) FROM merge_decisions'), 0, 'failed source row delete finalization records no decisions');
...
SQL);
    ...
    $wp_page_collision_base = $tmp . '/wp-page-collision-base.sqlite';
    $wp_page_collision_source = $tmp . '/wp-page-collision-source.sqlite';
    $wp_page_collision_target = $tmp . '/wp-page-collision-target.sqlite';
    create_base_db($wp_page_collision_base);
    $db = open_db($wp_page_collision_base);
    $db->exec('ALTER TABLE wp_posts ADD COLUMN post_parent INTEGER DEFAULT 0');
    $db->exec('CREATE TABLE wp_postmeta (meta_id INTEGER PRIMARY KEY AUTOINCREMENT, post_id INTEGER, meta_key TEXT, meta_value TEXT)');
    $db->exec('CREATE TABLE wp_comments (comment_ID INTEGER PRIMARY KEY AUTOINCREMENT, comment_post_ID INTEGER, comment_content TEXT)');
    $db->exec('CREATE TABLE wp_term_relationships (object_id INTEGER, term_taxonomy_id INTEGER, term_order INTEGER, PRIMARY KEY (object_id, term_taxonomy_id))');
    $db->close();
    copy($wp_page_collision_base, $wp_page_collision_source);
    copy($wp_page_collision_base, $wp_page_collision_target);
    $db = open_db($wp_page_collision_source);
    $db->exec("INSERT INTO wp_posts (ID, post_title, post_content, post_status, post_parent) VALUES (2, 'Source page', 'Created on feature', 'publish', 0)");
    $db->exec("INSERT INTO wp_posts (ID, post_title, post_content, post_status, post_parent) VALUES (3, 'Source child page', 'Created under feature page', 'publish', 2)");
    $db->exec("INSERT INTO wp_postmeta (meta_id, post_id, meta_key, meta_value) VALUES (1, 2, '_source_marker', 'source page meta')");
    $db->exec("INSERT INTO wp_postmeta (meta_id, post_id, meta_key, meta_value) VALUES (2, 3, '_source_child_marker', 'source child meta')");
    $db->exec("INSERT INTO wp_comments (comment_ID, comment_post_ID, comment_content) VALUES (1, 2, 'source page comment')");
    $db->exec("INSERT INTO wp_term_relationships (object_id, term_taxonomy_id, term_order) VALUES (2, 11, 0)");
    $db->close();
    $db = open_db($wp_page_collision_target);
    $db->exec("INSERT INTO wp_posts (ID, post_title, post_content, post_status, post_parent) VALUES (2, 'Target page', 'Created on main', 'publish', 0)");
    $db->exec("INSERT INTO wp_postmeta (meta_id, post_id, meta_key, meta_value) VALUES (1, 2, '_target_marker', 'target page meta')");
    $db->exec("INSERT INTO wp_comments (comment_ID, comment_post_ID, comment_content) VALUES (1, 2, 'target page comment')");
    $db->exec("INSERT INTO wp_term_relationships (object_id, term_taxonomy_id, term_order) VALUES (2, 12, 0)");
    $db->close();
    $wp_page_collision_result = cow_merge_databases($wp_page_collision_base, $wp_page_collision_source, $wp_page_collision_target, $metadata, 'feature-wp-page-collision', 'main');
    assert_same($wp_page_collision_result['status'], 'completed', 'independently inserted WordPress pages with the same ID merge without review conflicts');
    assert_same((int)scalar($wp_page_collision_target, "SELECT COUNT(*) FROM wp_posts WHERE post_title IN ('Source page', 'Target page')"), 2, 'source and target same-ID pages are both present after merge');
    $source_page_id = (int)scalar($wp_page_collision_target, "SELECT ID FROM wp_posts WHERE post_title = 'Source page'");
    assert_true($source_page_id > 0 && $source_page_id !== 2, 'source page receives a fresh target ID instead of overwriting the target page');
    assert_same((int)scalar($wp_page_collision_target, "SELECT COUNT(*) FROM wp_posts WHERE ID = 2 AND post_title = 'Target page'"), 1, 'target page keeps its original ID and payload');
    assert_same((int)scalar($wp_page_collision_target, "SELECT COUNT(*) FROM wp_postmeta WHERE post_id = $source_page_id AND meta_key = '_source_marker' AND meta_value = 'source page meta'"), 1, 'source page metadata is remapped to the fresh source page ID');
    assert_same((int)scalar($wp_page_collision_target, "SELECT COUNT(*) FROM wp_postmeta WHERE post_id = 2 AND meta_key = '_target_marker' AND meta_value = 'target page meta'"), 1, 'target page metadata stays attached to the target page');
    assert_same((int)scalar($wp_page_collision_target, "SELECT COUNT(*) FROM wp_comments WHERE comment_post_ID = $source_page_id AND comment_content = 'source page comment'"), 1, 'source page comments are remapped to the fresh source page ID');
    assert_same((int)scalar($wp_page_collision_target, "SELECT COUNT(*) FROM wp_comments WHERE comment_post_ID = 2 AND comment_content = 'target page comment'"), 1, 'target page comments stay attached to the target page');
    assert_same((int)scalar($wp_page_collision_target, "SELECT COUNT(*) FROM wp_term_relationships WHERE object_id = $source_page_id AND term_taxonomy_id = 11"), 1, 'source page taxonomy relationships are remapped to the fresh source page ID');
    assert_same((int)scalar($wp_page_collision_target, "SELECT COUNT(*) FROM wp_term_relationships WHERE object_id = 2 AND term_taxonomy_id = 12"), 1, 'target page taxonomy relationships stay attached to the target page');
    assert_same((int)scalar($wp_page_collision_target, "SELECT COUNT(*) FROM wp_posts WHERE post_title = 'Source child page' AND post_parent = $source_page_id"), 1, 'source child pages are reparented to the fresh source page ID');
    $source_child_page_id = (int)scalar($wp_page_collision_target, "SELECT ID FROM wp_posts WHERE post_title = 'Source child page'");
    assert_same((int)scalar($wp_page_collision_target, "SELECT COUNT(*) FROM wp_postmeta WHERE post_id = $source_child_page_id AND meta_key = '_source_child_marker' AND meta_value = 'source child meta'"), 1, 'source child page metadata remains attached after parent post rekeying');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_conflicts c JOIN merge_runs r ON r.id = c.run_id WHERE r.source_branch = 'feature-wp-page-collision'"), 0, 'WordPress same-ID page inserts do not leave merge conflicts');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions d JOIN merge_runs r ON r.id = d.run_id WHERE r.source_branch = 'feature-wp-page-collision' AND d.table_name = 'wp_posts' AND d.decision = 'source-applied' AND d.reason LIKE 'source inserted WordPress post with a colliding target ID%'"), 2, 'WordPress post rekeys are recorded in merge decisions');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions d JOIN merge_runs r ON r.id = d.run_id WHERE r.source_branch = 'feature-wp-page-collision' AND d.table_name IN ('wp_postmeta', 'wp_comments') AND d.decision = 'source-applied' AND d.reason LIKE 'source inserted WordPress child row with a colliding target ID%'"), 3, 'WordPress child row rekeys are recorded in merge decisions');
    ...
    $GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize'] = [
        static function (SQLite3Result $result, string $message): void {
            if ($message === 'failed to finalize audit table check') {
                throw new RuntimeException('forced audit table check finalize failure');
            }
        },
    ];
    assert_throws(
        fn() => cow_merge_audit_report($metadata, null, 5),
        'forced audit table check finalize failure',
        'merge audit table-check finalize failures surface to the caller'
    );
    unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize']);

    $GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize'] = [
        static function (SQLite3Result $result, string $message): void {
            if ($message === 'failed to finalize audit table inspection') {
                throw new RuntimeException('forced audit table inspection finalize failure');
            }
        },
    ];
    assert_throws(
        fn() => cow_merge_audit_report($metadata, null, 5),
        'forced audit table inspection finalize failure',
        'merge audit table-info finalize failures surface to the caller'
    );
    unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize']);

    $GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize'] = [
        static function (SQLite3Result $result, string $message): void {
            if ($message === 'failed to finalize audit query') {
                throw new RuntimeException('forced audit row finalize failure');
            }
        },
    ];
    assert_throws(
        fn() => cow_merge_audit_report($metadata, null, 5),
        'forced audit row finalize failure',
        'merge audit row-query finalize failures surface to the caller'
    );
    unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize']);
    ...
    $metadata_identity_prepare_failure = $tmp . '/metadata-identity-prepare-failure.sqlite';
    $metadata_identity_prepare_failure_db = open_db($metadata_identity_prepare_failure);
    cow_merge_ensure_metadata($metadata_identity_prepare_failure_db);
    $GLOBALS['cow_merge_test_hooks']['before_sqlite_prepare'] = [
        function (SQLite3 $db, string $sql, string $message) use ($metadata_identity_prepare_failure_db): void {
            if ($db === $metadata_identity_prepare_failure_db && $message === 'failed to prepare row identity upsert') {
                throw new RuntimeException('forced row identity prepare failure');
            }
        },
    ];
    assert_throws(
        fn() => cow_merge_remember_row_identity(
            $metadata_identity_prepare_failure_db,
            1,
            'feature-identity-prepare',
            'plugin_keyless_prepare',
            7,
            ['sidecar' => 'keyless-row', 'prepare' => 'failure'],
            ['label' => 'prepare failure']
        ),
        'forced row identity prepare failure',
        'row identity metadata prepare failures surface to the caller'
    );
    unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_prepare']);
    assert_same((int)scalar($metadata_identity_prepare_failure, 'SELECT COUNT(*) FROM merge_row_identities'), 0, 'failed row identity prepare records no sidecar rows');
    cow_merge_remember_row_identity(
        ...
        'plugin_keyless_prepare',
        7,
        ['sidecar' => 'keyless-row', 'prepare' => 'recovered'],
    ...
    $metadata_schema_finalize_failure = $tmp . '/metadata-schema-finalize-failure.sqlite';
    $metadata_schema_finalize_failure_db = open_db($metadata_schema_finalize_failure);
    $GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize'] = [
        function (SQLite3Result $result, string $message): void {
            if ($message === 'failed to finalize metadata table merge_runs inspection') {
                throw new RuntimeException('forced metadata schema finalize failure');
            }
        },
    ];
    assert_throws(
        fn() => cow_merge_ensure_metadata($metadata_schema_finalize_failure_db),
        'forced metadata schema finalize failure',
        'metadata table inspection finalize failures surface to the caller'
    );
    unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize']);
    assert_same((int)$metadata_schema_finalize_failure_db->querySingle("SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name LIKE 'merge_%'"), 0, 'failed metadata table inspection finalize rolls back metadata setup');
    cow_merge_ensure_metadata($metadata_schema_finalize_failure_db);
    assert_same(column_type($metadata_schema_finalize_failure, 'merge_runs', 'failure_reason'), 'TEXT', 'metadata schema creation succeeds after table inspection finalize failure is cleared');
    $metadata_schema_finalize_failure_db->close();
    ...
    $metadata_review_schema_finalize_failure = $tmp . '/metadata-review-schema-finalize-failure.sqlite';
    $metadata_review_schema_finalize_failure_db = open_db($metadata_review_schema_finalize_failure);
    $GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize'] = [
        function (SQLite3Result $result, string $message): void {
            if ($message === 'failed to finalize review-note metadata schema inspection') {
                throw new RuntimeException('forced review-note schema finalize failure');
            }
        },
    ];
    assert_throws(
        fn() => cow_merge_ensure_metadata($metadata_review_schema_finalize_failure_db),
        'forced review-note schema finalize failure',
        'review-note metadata schema finalize failures surface to the caller'
    );
    unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_result_finalize']);
    assert_same((int)$metadata_review_schema_finalize_failure_db->querySingle("SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name LIKE 'merge_%'"), 0, 'failed review-note schema finalize rolls back metadata setup');
    cow_merge_ensure_metadata($metadata_review_schema_finalize_failure_db);
    assert_same((int)$metadata_review_schema_finalize_failure_db->querySingle("SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'merge_review_notes'"), 1, 'metadata schema creation succeeds after review-note schema finalize failure is cleared');
    $metadata_review_schema_finalize_failure_db->close();
...
SQL);
    ...
    $source_empty_dir_identity = SQLite3::escapeString(cow_merge_file_identity_json('wp-content/uploads/source-empty-dir'));
    $source_changed_link_identity = SQLite3::escapeString(cow_merge_file_identity_json('wp-content/uploads/shared-link.txt'));
    $source_changed_link_payload = SQLite3::escapeString(cow_merge_payload_json(cow_merge_file_path_payload('wp-content/uploads/shared-link.txt', ['type' => 'symlink', 'target' => 'new-source.txt'])));
    $unchanged_target_link_payload = SQLite3::escapeString(cow_merge_payload_json(cow_merge_file_path_payload('wp-content/uploads/shared-link.txt', ['type' => 'symlink', 'target' => 'shared.txt'])));
    $source_link_file_identity = SQLite3::escapeString(cow_merge_file_identity_json('wp-content/uploads/links/source-link.txt'));
    ...
    $source_changed_file_payload = SQLite3::escapeString(cow_merge_payload_json(cow_merge_file_path_payload('wp-content/uploads/shared.txt', $merged_file_entries['wp-content/uploads/shared.txt'])));
    $unchanged_target_file_payload = SQLite3::escapeString(cow_merge_payload_json(cow_merge_file_path_payload('wp-content/uploads/shared.txt', $base_file_entries['wp-content/uploads/shared.txt'])));
    $source_delete_file_payload = SQLite3::escapeString(cow_merge_payload_json(cow_merge_file_path_payload('wp-content/uploads/delete-me.txt', $base_file_entries['wp-content/uploads/delete-me.txt'])));
    $source_delete_dir_payload = SQLite3::escapeString(cow_merge_payload_json(cow_merge_file_path_payload('wp-content/uploads/delete-empty-dir', $base_file_entries['wp-content/uploads/delete-empty-dir'])));
    $target_only_file_identity = SQLite3::escapeString(cow_merge_file_identity_json('wp-content/uploads/target-only.txt'));
    $target_only_file_payload = SQLite3::escapeString(cow_merge_payload_json(cow_merge_file_path_payload('wp-content/uploads/target-only.txt', $merged_file_entries['wp-content/uploads/target-only.txt'])));
    $target_delete_file_identity = SQLite3::escapeString(cow_merge_file_identity_json('wp-content/uploads/target-delete.txt'));
    $target_delete_base_payload = SQLite3::escapeString(cow_merge_payload_json(cow_merge_file_path_payload('wp-content/uploads/target-delete.txt', $base_file_entries['wp-content/uploads/target-delete.txt'])));
    ...
    $same_delete_base_payload = SQLite3::escapeString(cow_merge_payload_json(cow_merge_file_path_payload('wp-content/uploads/same-delete.txt', $base_file_entries['wp-content/uploads/same-delete.txt'])));
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_new_file_identity' AND reason = 'source added filesystem path and target did not have it' AND target_payload IS NULL"), 1, 'source-only filesystem addition records an empty target payload');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_new_file_identity' AND reason = 'source added filesystem path and target did not have it' AND base_payload IS NULL AND source_payload = '$source_new_file_payload' AND target_payload IS NULL AND chosen_payload = '$source_new_file_payload'"), 1, 'source-only filesystem addition records source and chosen payloads');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_changed_file_identity' AND reason = 'source changed filesystem path and target did not change it' AND source_payload = '$source_changed_file_payload' AND chosen_payload = '$source_changed_file_payload' AND target_payload = '$unchanged_target_file_payload'"), 1, 'source-only filesystem file change records source, chosen, and unchanged target payloads');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_changed_file_identity' AND reason = 'source changed filesystem path and target did not change it' AND base_payload = '$unchanged_target_file_payload' AND source_payload = '$source_changed_file_payload' AND target_payload = '$unchanged_target_file_payload' AND chosen_payload = '$source_changed_file_payload'"), 1, 'source-only filesystem file change records base and unchanged target payloads');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_empty_dir_identity' AND reason = 'source added filesystem directory and target did not have it' AND target_payload IS NULL"), 1, 'source-only filesystem directory addition records an empty target payload');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_empty_dir_identity' AND reason = 'source added filesystem directory and target did not have it' AND base_payload IS NULL AND source_payload = '$source_empty_dir_payload' AND target_payload IS NULL AND chosen_payload = '$source_empty_dir_payload'"), 1, 'source-only filesystem directory addition records source and chosen payloads');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_changed_link_identity' AND reason = 'source changed filesystem symlink target and target did not change it' AND source_payload = '$source_changed_link_payload' AND chosen_payload = '$source_changed_link_payload' AND target_payload = '$unchanged_target_link_payload'"), 1, 'source-only filesystem symlink target change records source, chosen, and unchanged target payloads');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_changed_link_identity' AND reason = 'source changed filesystem symlink target and target did not change it' AND base_payload = '$unchanged_target_link_payload' AND source_payload = '$source_changed_link_payload' AND target_payload = '$unchanged_target_link_payload' AND chosen_payload = '$source_changed_link_payload'"), 1, 'source-only filesystem symlink target change records base and unchanged target payloads');
    ...
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_link_file_identity' AND reason = 'source added filesystem symlink and target did not have it' AND base_payload IS NULL AND source_payload = '$source_link_file_payload' AND target_payload IS NULL AND chosen_payload = '$source_link_file_payload'"), 1, 'source-only filesystem symlink addition records source and chosen payloads');
    ...
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_delete_file_identity' AND reason = 'source deleted filesystem path and target did not change it' AND base_payload = '$source_delete_file_payload' AND source_payload IS NULL AND target_payload = '$source_delete_file_payload' AND chosen_payload IS NULL"), 1, 'source-only filesystem deletion records base and unchanged target payloads');
    ...
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$source_delete_dir_identity' AND reason = 'source deleted filesystem directory and target did not change it' AND base_payload = '$source_delete_dir_payload' AND source_payload IS NULL AND target_payload = '$source_delete_dir_payload' AND chosen_payload IS NULL"), 1, 'source-only filesystem directory deletion records base and unchanged target payloads');
    ...
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'target-kept' AND row_identity = '$target_only_file_identity' AND reason = 'target added filesystem path while source did not have it' AND base_payload IS NULL AND source_payload IS NULL AND target_payload = '$target_only_file_payload' AND chosen_payload = '$target_only_file_payload'"), 1, 'target-only filesystem addition records empty base and source payloads with preserved target and chosen payloads');
    ...
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'target-kept' AND row_identity = '$target_delete_file_identity' AND reason = 'target deleted filesystem path while source did not change it' AND base_payload = '$target_delete_base_payload' AND source_payload = '$target_delete_base_payload' AND target_payload IS NULL AND chosen_payload IS NULL"), 1, 'target-only filesystem deletion records unchanged source and empty chosen payloads');
    ...
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'target-kept' AND row_identity = '$target_change_file_identity' AND reason = 'target changed filesystem path while source did not change it' AND base_payload = '$target_change_base_payload' AND source_payload = '$target_change_base_payload' AND target_payload = '$target_change_file_payload' AND chosen_payload = '$target_change_file_payload'"), 1, 'target-only filesystem path change records unchanged source and preserved target payloads');
    ...
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_decisions WHERE table_name = '__files__' AND decision = 'source-applied' AND row_identity = '$same_added_file_identity' AND reason = 'source and target added the same filesystem path' AND base_payload IS NULL AND source_payload = '$same_added_file_payload' AND target_payload = '$same_added_file_payload' AND chosen_payload = '$same_added_file_payload'"), 1, 'identical source/target filesystem addition records matching source, target, and chosen payloads');
...
SQL);
    ...
    $file_commit_rollback_conflict_id = (int)scalar($metadata, "SELECT id FROM merge_conflicts WHERE table_name = '__files__' AND row_identity = '" . SQLite3::escapeString(cow_merge_file_identity_json('wp-content/uploads/commit-rollback-conflict.txt')) . "' ORDER BY id DESC LIMIT 1");
    $GLOBALS['cow_merge_test_hooks']['before_sqlite_exec'] = [
        static function (SQLite3 $db, string $sql, string $message): void {
            if ($sql === 'COMMIT' && $message === 'failed to commit filesystem resolution metadata transaction') {
                throw new RuntimeException('forced filesystem resolution metadata commit failure');
            }
        },
    ];
    $file_commit_resolution_failed = false;
    try {
        cow_merge_resolve_conflict(
            $metadata,
            $file_commit_rollback_conflict_id,
            'source',
            true,
            'Try audited source file with failing metadata commit.',
            'cow-test'
        );
    } catch (Throwable $e) {
        $file_commit_resolution_failed = str_contains($e->getMessage(), 'forced filesystem resolution metadata commit failure');
    } finally {
        unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_exec']);
    }
    assert_true($file_commit_resolution_failed, 'filesystem resolution metadata commit failure is surfaced to the caller');
    assert_same(file_get_contents($file_resolve_target_root . '/wp-content/uploads/commit-rollback-conflict.txt'), 'target commit rollback resolution', 'failed filesystem resolution metadata commit restores the target file');
    assert_same((int)scalar($metadata, "SELECT COUNT(*) FROM merge_resolutions WHERE conflict_id = $file_commit_rollback_conflict_id"), 0, 'failed filesystem resolution metadata commit records no resolution metadata');
    assert_same(
        (int)scalar($metadata, "SELECT COUNT(*) FROM merge_review_notes WHERE record_type = 'conflict' AND record_id = $file_commit_rollback_conflict_id"),
        0,
        'failed filesystem resolution metadata commit rolls back the reviewed conflict note'
    );
...
SQL);
    ...
    $rollback_artifact_root = $tmp . '/rollback-failure-artifact-root';
    write_test_file($rollback_artifact_root . '/wp-content/uploads/artifact.txt', 'artifact base');
    $rollback_artifact_fs_snapshot = cow_merge_file_root_snapshot_begin($rollback_artifact_root);
    $artifact_path = cow_merge_record_rollback_failure_artifact(
        ...
        [
            'target_db_snapshot' => cow_merge_sqlite_snapshot_artifact($rollback_artifact_db_snapshot),
            'filesystem_snapshot' => cow_merge_file_root_snapshot_artifact($rollback_artifact_fs_snapshot, $rollback_artifact_root),
        ]
    );
    ...
    assert_true(is_file($artifact_record['artifacts']['target_db_snapshot']['backup'] ?? ''), 'rollback failure artifact points to preserved target DB backup');
    assert_true(is_dir($artifact_record['artifacts']['filesystem_snapshot']['stage_root'] ?? ''), 'rollback failure artifact points to preserved filesystem backup directory');
    $artifact_file_backup = null;
    foreach ($artifact_record['artifacts']['filesystem_snapshot']['backups'] ?? [] as $backup) {
        if (($backup['path'] ?? '') === 'wp-content/uploads/artifact.txt') {
            $artifact_file_backup = $backup;
            break;
        }
    }
    assert_true(is_array($artifact_file_backup) && is_file($artifact_file_backup['backup_file'] ?? ''), 'rollback failure artifact maps filesystem backup files to original paths');
...
SQL);
    ...
    $band_prepare_failure_db = $tmp . '/band-prepare-failure.sqlite';
    $band_prepare_failure_metadata = $tmp . '/.forkpress/cow/merge/band-prepare-failure-metadata.sqlite';
    copy($band_base, $band_prepare_failure_db);
    $GLOBALS['cow_merge_test_hooks']['before_sqlite_prepare'] = [
        static function (SQLite3 $db, string $sql, string $message): void {
            if ($message === 'failed to prepare AUTOINCREMENT band selection') {
                throw new RuntimeException('forced AUTOINCREMENT band prepare failure');
            }
        },
    ];
    assert_throws(
        fn() => cow_merge_allocate_autoincrement_bands(
            $band_prepare_failure_db,
            $band_prepare_failure_metadata,
            'feature-band-prepare-failure'
        ),
        'forced AUTOINCREMENT band prepare failure',
        'AUTOINCREMENT metadata prepare failures surface to the caller'
    );
    unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_prepare']);
    ...
    $GLOBALS['cow_merge_test_hooks']['before_sqlite_exec'] = [
        static function (SQLite3 $db, string $sql, string $message): void {
            if ($sql === 'COMMIT' && $message === 'failed to commit AUTOINCREMENT metadata transaction') {
                throw new RuntimeException('forced AUTOINCREMENT metadata commit failure');
            }
        },
    ];
    $band_metadata_commit_failure_message = null;
    set_error_handler(static function (int $severity, string $message): bool {
        return str_contains($message, 'forced AUTOINCREMENT metadata commit failure');
    });
    try {
        cow_merge_allocate_autoincrement_bands(
            $band_metadata_commit_rollback_db,
            $band_metadata_commit_rollback_metadata,
            'feature-band-metadata-commit-rollback'
        );
    } catch (Throwable $e) {
        $band_metadata_commit_failure_message = $e->getMessage();
    } finally {
        unset($GLOBALS['cow_merge_test_hooks']['before_sqlite_exec']);
        restore_error_handler();
    }
    assert_true($band_metadata_commit_failure_message !== null, 'AUTOINCREMENT metadata commit failure is surfaced to the caller');
    assert_same((int)scalar($band_metadata_commit_rollback_db, "SELECT seq FROM sqlite_sequence WHERE name = 'plugin_autoinc'"), 1, 'failed AUTOINCREMENT metadata commit restores the already-committed sqlite_sequence changes');
    assert_same((int)scalar($band_metadata_commit_rollback_metadata, "SELECT COUNT(*) FROM merge_autoincrement_bands WHERE branch_name = 'feature-band-metadata-commit-rollback'"), 0, 'failed AUTOINCREMENT metadata commit records no band metadata');

...
SQL);
    ...
    assert_same($file_capture['files'], 10, 'filesystem merge base excludes ForkPress-managed files');
    ...
    assert_same(count($file_conflict_audit['conflicts']), 5, 'merge audit can focus on filesystem conflicts');
    ...
    assert_same(count(array_filter($file_conflict_audit['conflicts'], fn($row) => $row['table_name'] === '__files__')), 5, 'filesystem audit filter exports only file records');
...
SQL);
    ...
    create_base_db($band_ref_base);
    $db = open_db($band_ref_base);
    $db->exec('CREATE TABLE wp_postmeta (meta_id INTEGER PRIMARY KEY AUTOINCREMENT, post_id INTEGER, meta_key TEXT, meta_value TEXT)');
    $db->close();
    copy($band_ref_base, $band_ref_source);
    copy($band_ref_base, $band_ref_target);
    cow_merge_allocate_autoincrement_bands($band_ref_source, $band_metadata, 'feature-band-ref-source');
    cow_merge_allocate_autoincrement_bands($band_ref_target, $band_metadata, 'feature-band-ref-target');
    $db = open_db($band_ref_source);
    $db->exec("INSERT INTO wp_posts (post_title, post_content, post_status) VALUES ('Banded source ref', '', 'publish')");
    $band_ref_source_id = (int)$db->lastInsertRowID();
    $band_ref_source_json = json_encode(['linkedPostId' => $band_ref_source_id, 'branch' => 'source'], JSON_UNESCAPED_SLASHES);
    $band_ref_source_serialized = 'a:2:{s:12:"linkedPostId";i:' . $band_ref_source_id . ';s:6:"branch";s:6:"source";}';
    $stmt = $db->prepare('UPDATE wp_posts SET post_content = :content WHERE ID = :id');
    $stmt->bindValue(':content', $band_ref_source_json, SQLITE3_TEXT);
    $stmt->bindValue(':id', $band_ref_source_id, SQLITE3_INTEGER);
    $stmt->execute();
    $stmt = $db->prepare("INSERT INTO wp_postmeta (post_id, meta_key, meta_value) VALUES (:id, '_forkpress_json_ref', :json), (:id, '_forkpress_serialized_ref', :serialized)");
    $stmt->bindValue(':id', $band_ref_source_id, SQLITE3_INTEGER);
    $stmt->bindValue(':json', $band_ref_source_json, SQLITE3_TEXT);
    $stmt->bindValue(':serialized', $band_ref_source_serialized, SQLITE3_TEXT);
    $stmt->execute();
    $db->close();
    $db = open_db($band_ref_target);
    $db->exec("INSERT INTO wp_posts (post_title, post_content, post_status) VALUES ('Banded target ref', '', 'publish')");
    $band_ref_target_id = (int)$db->lastInsertRowID();
    $band_ref_target_json = json_encode(['linkedPostId' => $band_ref_target_id, 'branch' => 'target'], JSON_UNESCAPED_SLASHES);
    $band_ref_target_serialized = 'a:2:{s:12:"linkedPostId";i:' . $band_ref_target_id . ';s:6:"branch";s:6:"target";}';
    $stmt = $db->prepare('UPDATE wp_posts SET post_content = :content WHERE ID = :id');
    $stmt->bindValue(':content', $band_ref_target_json, SQLITE3_TEXT);
    $stmt->bindValue(':id', $band_ref_target_id, SQLITE3_INTEGER);
    $stmt->execute();
    $stmt = $db->prepare("INSERT INTO wp_postmeta (post_id, meta_key, meta_value) VALUES (:id, '_forkpress_json_ref', :json), (:id, '_forkpress_serialized_ref', :serialized)");
    $stmt->bindValue(':id', $band_ref_target_id, SQLITE3_INTEGER);
    $stmt->bindValue(':json', $band_ref_target_json, SQLITE3_TEXT);
    $stmt->bindValue(':serialized', $band_ref_target_serialized, SQLITE3_TEXT);
    $stmt->execute();
    $db->close();
    assert_true($band_ref_source_id !== $band_ref_target_id, 'banded WordPress branches assign distinct IDs before serialized references are written');
    $band_ref_result = cow_merge_databases($band_ref_base, $band_ref_source, $band_ref_target, $band_metadata, 'feature-band-ref-source', 'feature-band-ref-target');
    assert_same($band_ref_result['status'], 'completed', 'banded WordPress reference rows merge without ID collision repair');
    assert_same(scalar($band_ref_target, "SELECT post_content FROM wp_posts WHERE ID = $band_ref_source_id"), $band_ref_source_json, 'source JSON post reference remains valid after banded merge');
    assert_same(scalar($band_ref_target, "SELECT post_content FROM wp_posts WHERE ID = $band_ref_target_id"), $band_ref_target_json, 'target JSON post reference remains valid after banded merge');

...
php -r '$db = new SQLite3($argv[1]); $conflicts = (int)$db->querySingle("SELECT COUNT(*) FROM merge_conflicts c JOIN merge_runs r ON r.id = c.run_id WHERE c.table_name = '\''wp_forkpress_e2e_keyless_unique_same'\'' AND c.conflict_type = '\''row-unique-collision'\'' AND r.source_branch = '\''keyless-unique-same'\'' AND r.target_branch = '\''main'\''"); $decisions = (int)$db->querySingle("SELECT COUNT(*) FROM merge_decisions d JOIN merge_runs r ON r.id = d.run_id WHERE d.table_name = '\''wp_forkpress_e2e_keyless_unique_same'\'' AND d.decision = '\''source-applied'\'' AND d.reason LIKE '\''source inserted no-primary-key row already exists in target by unique index%'\'' AND r.source_branch = '\''keyless-unique-same'\'' AND r.target_branch = '\''main'\''"); exit($conflicts === 0 && $decisions === 1 ? 0 : 1);' "$WORK_DIR/cow/merge/metadata.sqlite"

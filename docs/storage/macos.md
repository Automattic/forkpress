...
ForkPress uses APFS copy-on-write on macOS. It first tries native
`clonefile` inside the project directory. If the current volume cannot clone
files, ForkPress falls back to a rootless APFS sparsebundle.

## APFS `clonefile`

`clonefile` creates a new file whose data blocks initially share APFS extents
...
When one branch writes to the file, APFS allocates new blocks for the
changed data:
...
You get independent ordinary paths with shared unchanged file contents.
...
Physical branch trees live in the mounted APFS image. Public branch
directories such as `./main` and `./marketing` remain visible beside
`.forkpress`.

## Mount lifecycle

Stop ForkPress before deleting or moving a sparsebundle-backed site:
...
`.forkpress/macos-cow/mount` and run `forkpress stop` again. Use `--force`
only after a normal detach reports a busy mount.

...
goal is ordinary paths that work with editors, PHP, Git, WP-CLI, backup
tools, and shell commands.

## Native file cloning

Native cloning is the default storage engine whenever the project directory
supports it.
...
Each cloned file starts by sharing physical storage with its source. When
one branch writes to the file, the filesystem allocates new blocks for the
changed data. Branch paths remain independent.
...
The Linux XFS loop fallback provides a reflink-capable filesystem even when
the project filesystem cannot reflink files. ForkPress stores physical
branch trees inside one shared mounted XFS volume and exposes public branch
...
block cloning through ordinary Win32 paths. There is no requirement for WSL,
Docker, FUSE, WinFsp, or developer-only storage tools.

## File-copy materialization

Full file-copy materialization is available only when explicitly requested.
It does not share physical blocks and is not part of the automatic
copy-on-write cascade.

## Not used in production

Production ForkPress does not use SQL overlay views, branch table prefixes,
Docker, a MySQL daemon, FUSE mounts, helper daemons, or lazy virtual

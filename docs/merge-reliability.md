...
- Source child-row deletes for target-referenced WordPress owners now inherit
  ...
  removed.
- Built-in WordPress semantic validation now covers stale `post_parent`
  references from revisions as well as child pages and attachments, so a
  source-deleted parent post cannot leave target-edited revision rows dangling
...
- The Events Calendar-shaped validator also covers plugin option/cache JSON:
  `tribe_events_recent_event_ids` can preserve target-side option edits while
  holding stale deleted-event IDs as plugin-scoped conflicts with logical
  identities.
...
- Keep aarch64 macOS release artifacts and APFS sparsebundle E2E runs green for
  each release. `v0.1.36` is the current published release after all five
  production-bundle targets stayed green and the release commit
  `2154ad3fd801b824d9bc0d251f9acea0553755a0` was published.

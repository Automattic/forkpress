...
- `tests/cow/merge_smoke.php` now also fast-gates the clean counterpart:
  independent source and target `wp_global_styles` rows with distinct style
  keys merge with zero conflicts and audit both sides. The same smoke gate
  keeps source-edited/target-deleted `wp_global_styles` rows reviewable.

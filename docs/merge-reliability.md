...
It also includes page/postmeta, user, taxonomy term, reusable block, menu,
attachment, and option edit/delete conflict cases that must stay reviewable and
keep the target deletion until review. The page/postmeta case covers the
`wp_posts` row and its `wp_postmeta` graph, the user case covers `wp_users` and
...
and the option case covers JSON and serialized `wp_options` rows.

...
relative symlinks, unsafe absolute/self-referential/managed-path symlink
conflicts, directory/file type replacement review, and source directory deletes
with target descendants, run:
...
original/generated dimensions, attached-file metadata drift, generated-size
...
validator runner contract checks, rerun replacement evidence, and explicit
plugin source-evidence drift, run:
...
- Keep aarch64 macOS release artifacts and APFS sparsebundle E2E runs green for
  each release. `v0.1.16` is the first published release after the Apple Silicon
  production-bundle gate was restored.

...
| --- | --- | --- |
| WordPress semantic objects | Tests cover real post creation, postmeta references, branch-local page edits/deletes, attachment uploads plus files, hierarchical taxonomy terms, nav menus with menu-location assignments, reusable blocks, options with embedded object IDs, JSON option payloads with embedded object IDs, custom post types, plugin AUTOINCREMENT tables, keyless plugin tables, unique collisions, file additions, and branch merge visibility. | Add deeper concurrent edit/delete matrices, synced-pattern/block reference rewrites, attachment metadata regeneration, generated media sizes, and nested plugin-owned JSON/serialized graphs spanning custom tables and files. |
...
| Filesystem semantics | File additions/deletions/conflicts are audited; binary file changes/conflicts are hash-verified, symlink policy is tested, unsafe entries remain conflicts, directory/file replacements get type-specific review conflicts, unchanged target descendants under reviewed replacements are preserved, and reviewed source replacements can apply supported file/dir/symlink changes. | Add uploads-specific checks that connect attachment rows to files and generated asset handling. |

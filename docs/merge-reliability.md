...
`post_mime_type` drift against known upload file extensions, attached-file metadata drift, generated-size
...
- Keep aarch64 macOS release artifacts and APFS sparsebundle E2E runs green for
  each release. `v0.1.28` is the current published release after the Apple
  Silicon production-bundle gate stayed green, the no-header branch UI action
  regression fix was included, and independently inserted `theme_mods_*` rows
  merge without dropping branch-local menu locations.

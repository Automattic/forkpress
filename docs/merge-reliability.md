...
| Crash consistency | DB, metadata, file, rollback-failure, ID-band, and Git publication paths have targeted rollback tests. DB merge process-death coverage includes crashes before/after target DB commit and before metadata commit, with pending crash artifacts that block later merges until explicit recovery. Whole-branch DB+file merges now leave a recoverable target DB, metadata DB, and filesystem-root snapshot before the file phase starts, so a hard exit after DB commit but before file mutation blocks later merges until `recover-crash --restore-target-db --restore-files` restores a coherent pre-merge state. `docs/merge-crash-consistency.md` maps covered boundaries and missing process-death failpoints. | Add external failpoint tests around later file publish boundaries with DB changes, sparsebundle detach/compact, Git publication, pruning, and cleanup. |

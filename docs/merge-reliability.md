...
Recent additions released in `v0.1.32`:
...
page-plus-comment, page-plus-custom-post-type, page-plus-taxonomy,
page-plus-menu, page-plus-reusable-block, page-plus-attachment,
page-plus-image-block, page-plus-gallery, page-plus-file-block,
page-plus-media-text, and page-plus-options cases must complete with zero conflicts while preserving
branch-specific IDs inside comment/user graphs, plugin-like CPT graphs, menu
relationships, theme-mod locations, reusable-block comments, attachment
metadata, `core/image`/`core/gallery`/`core/file`/`core/media-text` block JSON,
upload files, JSON payloads, and serialized payloads without rewrite.
It also includes reusable block, menu, and attachment edit/delete conflict cases
that must stay reviewable and keep the target deletion until review. The
reusable block case covers the deleted `wp_block` row and target page cleanup,
the menu case covers menu terms, taxonomy rows, menu item posts, menu item
metadata, relationships, and theme-mod location cleanup, and the attachment
case covers the DB and file graph.
...
attachment metadata rows, missing original, metadata-side original, `original_image`, or generated upload files, invalid serialized attachment
metadata, duplicate original/generated upload ownership, invalid
original/generated dimensions, original/generated filesize drift, attachment
`post_mime_type` and generated-size `mime-type` drift against known upload file extensions, attached-file metadata drift, generated-size
or `original_image` filename drift, unsafe primary/metadata/generated/`original_image` paths, and malformed or
incomplete generated-size metadata, run:
...
deleted reusable blocks or synced patterns, child pages or attachments left
...
deleted attachments/files, `core/query` block JSON left pointing at deleted author
users or taxonomy terms, `core/avatar` block JSON left pointing at deleted
users, `core/navigation-link` block JSON left pointing at deleted pages or
...
- Keep aarch64 macOS release artifacts and APFS sparsebundle E2E runs green for
  each release. `v0.1.32` is the current published release after all five
  production-bundle targets stayed green and the current reliability slices
  through trunk commit `21907029759f640c4134d31f3b90b8cbc438cb16` were
  released.

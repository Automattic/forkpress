...
It also includes user, taxonomy term, reusable block, menu, and attachment
edit/delete conflict cases that must stay reviewable and keep the target
deletion until review. The user case covers `wp_users` and `wp_usermeta`, the
taxonomy case covers term rows, term-taxonomy rows, termmeta, and page-term
relationships, the reusable block case covers the deleted `wp_block` row and
target page cleanup, the menu case covers menu terms, taxonomy rows, menu item
posts, menu item metadata, relationships, and theme-mod location cleanup, and
the attachment case covers the DB and file graph.

...
| Plugin-specific semantics | Generic SQLite merge is table/row/cell based and does not rewrite embedded IDs. `docs/plugin-merge-validators.md` defines the validator boundary and first test shape. E2E covers the clean happy path for a plugin-owned custom-table graph with JSON, serialized option/postmeta references, and a referenced file. | Implement the validator API, plugin-scoped audit records, and plugin-owned graph fixtures that intentionally break references or conflict with target state. |

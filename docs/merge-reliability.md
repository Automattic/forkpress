...
page-plus-image-block, page-plus-gallery, page-plus-file-block, and
page-plus-options cases must complete with zero conflicts while preserving
...
metadata, `core/image`/`core/gallery`/`core/file` block JSON, upload files,
JSON payloads, and serialized payloads without rewrite.
...
requests that should be answered before WordPress `admin-post.php` bootstrap,
run:
...
- Keep aarch64 macOS release artifacts and APFS sparsebundle E2E runs green for
  each release. `v0.1.25` is the current published release after the Apple
  Silicon production-bundle gate was restored and the follow-up merge
  reliability slices were included.

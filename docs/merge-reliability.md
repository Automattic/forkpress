...
It also includes menu and attachment edit/delete conflict cases that must stay
reviewable and keep the target deletion until review. The menu case covers menu
...
WordPress `admin-post.php` bootstrap, run:
...
- Keep aarch64 macOS release artifacts and APFS sparsebundle E2E runs green for
  each release. `v0.1.26` is the current published release after the Apple
  Silicon production-bundle gate stayed green and the no-header branch UI
  action regression fix was included.

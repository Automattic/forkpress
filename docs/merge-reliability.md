...
| Crash consistency | DB, metadata, file, rollback-failure, ID-band, and Git publication paths have targeted rollback tests. DB merge process-death coverage includes crashes before/after target DB commit and before metadata commit, with pending crash artifacts that block later merges until explicit recovery. `docs/merge-crash-consistency.md` maps covered boundaries and missing process-death failpoints. | Add external failpoint tests around file publish, sparsebundle detach/compact, Git publication, pruning, and cleanup. |

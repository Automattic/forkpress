...
- Keep conflict resolution modeled as a first-class contract in audit output:
  ...
  should offer next, and `merge-audit --latest-revalidation-status <status>`
  can focus conflicts whose latest revalidation guard is still current or has
  drifted again. `merge-audit --records conflicts --group-by lifecycle`,
  `--group-by next-action`, `--group-by conflict-key`, and `--group-by
  latest-revalidation-status` can summarize queue counts by lifecycle, required
  action, logical conflict, or current guard status without client-side
  aggregation.

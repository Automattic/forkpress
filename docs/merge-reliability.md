...
- Built-in WordPress semantic validator findings now carry
  `semantic_scope=wordpress`, while ordinary plugin validator findings default
  to `semantic_scope=plugin`. `merge-audit --semantic-scope wordpress|plugin`
  gives UI and CLI queues a stable way to separate core WordPress semantic
  review from plugin-owned validator review without changing merge behavior.

...
relative symlinks, directory/file type replacement review, source directory
deletes with target descendants, and unsafe symlink conflicts, run:
...
For explicit import IDs or primary-key rewrites that fall outside a branch's
...
For branch-birth metadata validation and cleanup isolation without the Git
publication harness, run:
...
source-added views/triggers, cyclic source-added views/triggers, or
source-added triggers with missing dependencies, run:

```bash
make test-cow-schema-review
```

For WordPress upload/media validator changes, including missing original or
generated upload files, invalid serialized attachment metadata, duplicate
original/generated upload ownership, invalid original/generated dimensions,
attached-file metadata drift, generated-size filename drift, unsafe
primary/generated paths, and incomplete generated-size metadata, run:
...
attachments/files, and term relationships left pointing at deleted taxonomy
terms, plus options/widgets/theme mods left pointing at deleted WordPress
objects, run:
...
or JSON option/postmeta references, validator runner contract checks, and
rerun replacement evidence, run:
...
no-primary-key rowid reuse, and guarded `--after-revalidate` resolution
changes, run:

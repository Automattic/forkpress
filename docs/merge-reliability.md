...
This smoke gate includes the basic regression for a page created on a feature
branch and a different page created on `main`: the merge must complete with
zero conflicts and the merged target must contain both pages.
...
- Keep aarch64 macOS release artifacts and APFS sparsebundle E2E runs green for
  each release. `v0.1.24` is the current published release after the Apple

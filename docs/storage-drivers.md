...
| Doltlite | Experimental runtime path. `strategy = "doltlite"` stores BranchFS file tables and WordPress tables in one `.forkpress/doltlite/site.doltlite` database and opens requests through Doltlite's `site.doltlite@<branch>` selector. | Native Doltlite branches apply to both WordPress files and WordPress tables. There are no ForkPress SQL COW views for WordPress tables, and file blobs stay in the database like the default BranchFS driver. Git smart HTTP is wired through a Doltlite-aware adapter that opens each branch directly. | Requires the bundled PHP SQLite/PDO SQLite symbols to resolve to Doltlite rather than stock SQLite. It still uses the BranchFS PHP interception surface for WordPress files. The Git adapter treats `database.sql` as read-only context, and rollback for failed pushes needs a fuller Doltlite reset-to-ref hook. |

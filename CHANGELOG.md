...
- Added COW coverage proving file-scoped unreviewed merge-audit review queues mark matching text records.
...
- Added COW coverage proving source-only filesystem file changes record base and unchanged target payloads.
- Added COW coverage proving source-only filesystem directory additions record source and chosen payloads.
- Added COW coverage proving source-only filesystem symlink additions record source and chosen payloads.
- Added COW coverage proving source-only filesystem directory deletions record base and unchanged target payloads.
...
- Added COW coverage proving source-only filesystem file changes record source, chosen, and unchanged target payloads.
- Added COW coverage proving target-only filesystem deletions record unchanged source and empty chosen payloads.
- Added COW coverage proving target-only filesystem path changes record unchanged source and preserved target payloads.
- Added COW coverage proving target-only filesystem additions record empty base and source payloads with preserved target and chosen payloads.
- Added COW coverage proving source-only filesystem symlink target changes record source, chosen, and unchanged target payloads in merge audit decisions.
- Added COW coverage proving reviewed source-applied filesystem deletion decisions stay visible through path-scoped audit review filters.
- Added COW coverage proving source-only filesystem directory deletions record empty source and chosen payloads in merge audit decisions.
- Added COW coverage proving source-only filesystem deletions record empty source and chosen payloads in merge audit decisions.
- Added COW coverage proving source-only filesystem symlink additions record an empty target payload in merge audit decisions.
...
- Tightened foreign-key parent/child lookup I/O so infrastructure failures surface and roll back instead of becoming misleading `row-target-constraint` conflicts.

...
- `forkpress branch merge-resolve conflict <id> --choice source|target [--apply]`
  `[--note TEXT] [--reviewer NAME]` validates an audited explicit-PK DB cell or
  row insert-collision conflict and, with `--apply`, records the deterministic
  resolution in merge metadata.

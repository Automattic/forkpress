...
function cow_merge_sqlite_snapshot_changed(?array $snapshot): bool {
    if ($snapshot === null) {
        return false;
    }
    $path = (string)($snapshot['path'] ?? '');
    ...
    $current = cow_merge_temp_sqlite_path(dirname($path), '.forkpress-merge-db-current-');
    try {
        cow_merge_backup_sqlite_db($path, $current);
        return hash_file('sha256', $backup) !== hash_file('sha256', $current);
    } finally {
        @unlink($current);
    }
}
...
function cow_merge_file_root_snapshot_changed(?array $snapshot, string $target_root): bool {
    if ($snapshot === null || $target_root === '') {
        return false;
    }
    ...
    return cow_merge_file_manifest_for_root($target_root)['entries'] !== $original_entries;
}
...
function cow_merge_plugin_validator_semantic_scope(array $finding, string $plugin): string {
    if (array_key_exists('semantic_scope', $finding)) {
        if (!is_string($finding['semantic_scope'])) {
            throw new InvalidArgumentException('plugin validator semantic_scope must be a string');
        }
        $scope = trim((string)$finding['semantic_scope']);
        if ($scope === '') {
            throw new InvalidArgumentException('plugin validator semantic_scope must not be empty');
        }
        if (!in_array($scope, ['wordpress', 'plugin'], true)) {
            throw new InvalidArgumentException('plugin validator semantic_scope must be wordpress or plugin');
        }
        return $scope;
    }
    return str_starts_with($plugin, 'forkpress-wp-') ? 'wordpress' : 'plugin';
}
    ...
    try {
        ...
        foreach ($findings as $finding) {
            ...
            $payload = [
                ...
                'semantic_scope' => cow_merge_plugin_validator_semantic_scope($finding, $plugin),
...
function cow_merge_plugin_driver_command(string $driver): array {
    $driver = trim($driver);
    if ($driver === '') {
        throw new InvalidArgumentException('--driver is required');
    }
    if (!is_file($driver)) {
        throw new InvalidArgumentException("--driver must point to a file: $driver");
    }
    if (strtolower(pathinfo($driver, PATHINFO_EXTENSION)) === 'php') {
        return [PHP_BINARY, $driver];
    }
    if (!is_executable($driver)) {
        throw new InvalidArgumentException("--driver must be executable or a PHP script: $driver");
    }
    return [$driver];
}

function cow_merge_decode_plugin_driver_stdout(string $stdout, string $driver): array {
    $decoded = json_decode($stdout, true);
    if (json_last_error() !== JSON_ERROR_NONE) {
        $preview = trim($stdout);
        if (strlen($preview) > 240) {
        ...
        throw new RuntimeException(
            "plugin driver $driver did not emit valid JSON: "
            . json_last_error_msg()
            . ($preview === '' ? '' : "; stdout: $preview")
        );
    }
    if (!is_array($decoded) || array_is_list($decoded)) {
        throw new RuntimeException("plugin driver $driver must emit a JSON object");
    }
    $status = (string)($decoded['status'] ?? '');
    if (!in_array($status, ['validated', 'applied', 'failed'], true)) {
        throw new RuntimeException("plugin driver $driver emitted an invalid status");
    }
    if ($status === 'failed') {
        $reason = trim((string)($decoded['reason'] ?? ''));
        throw new RuntimeException("plugin driver $driver failed" . ($reason === '' ? '' : ": $reason"));
    }
    if (!array_key_exists('result', $decoded)) {
        throw new RuntimeException("plugin driver $driver must emit a result value");
    }
    $note = null;
    if (array_key_exists('note', $decoded)) {
        if (!is_string($decoded['note'])) {
            throw new RuntimeException("plugin driver $driver note must be a string");
        }
        $note = trim($decoded['note']);
        if ($note === '') {
            throw new RuntimeException("plugin driver $driver note must not be empty");
        }
    }
    return [
        ...
        'previous' => $decoded['previous'] ?? null,
        'note' => $note,
    ];
}

function cow_merge_plugin_driver_conflict_context(SQLite3 $meta, int $conflict_id): array {
    $stmt = cow_merge_prepare_checked(
        $meta,
    ...
    cow_merge_bind($stmt, ':id', $conflict_id);
    $res = cow_merge_execute_checked($stmt, $meta, 'failed to read plugin driver context');
    $row = $res->fetchArray(SQLITE3_ASSOC);
    cow_merge_result_finalize_checked($res, 'failed to finalize plugin driver context lookup');
    if (!$row) {
        throw new InvalidArgumentException("conflict #$conflict_id does not exist in merge metadata");
    }
    if ((string)$row['table_name'] !== '__plugins__') {
        throw new InvalidArgumentException('run-plugin-driver only supports plugin validator conflicts');
    }
    ...
    return [
        ...
        'run_id' => (int)$row['run_id'],
        'conflict_id' => (int)$row['id'],
        'conflict_key' => $row['conflict_key'] ?? null,
        'previous_conflict_id' => isset($row['previous_conflict_id']) ? (int)$row['previous_conflict_id'] : null,
        'table_name' => (string)$row['table_name'],
        'row_identity' => cow_merge_decode_payload_json((string)$row['row_identity'], 'plugin conflict identity'),
        'column_name' => (string)($row['column_name'] ?? ''),
        'conflict_type' => (string)$row['conflict_type'],
        'plugin' => is_array($plugin_payload) ? [
            'plugin' => $plugin_payload['plugin'] ?? null,
            'object' => $plugin_payload['object'] ?? null,
            'validator' => $plugin_payload['validator'] ?? null,
            'severity' => $plugin_payload['severity'] ?? null,
            'logical_identity' => $plugin_payload['logical_identity'] ?? null,
        ] : null,
        'payloads' => [
            'base' => cow_merge_decode_payload_json((string)$row['base_payload'], 'plugin conflict base'),
            'source' => cow_merge_decode_payload_json((string)$row['source_payload'], 'plugin conflict source'),
            'target' => cow_merge_decode_payload_json((string)$row['target_payload'], 'plugin conflict target'),
            'chosen' => $plugin_payload,
        ],
        'merge' => [
            'source_branch' => (string)$row['source_branch'],
            'target_branch' => (string)$row['target_branch'],
...
function cow_merge_require_current_plugin_validator_conflict(SQLite3 $meta, array $conflict, string $command): void {
    ...
    if ($status === 'stale') {
        $replacement = isset($staleness['replacement_conflict_id'])
            ? ' #' . (int)$staleness['replacement_conflict_id']
            : '';
        throw new RuntimeException("$command cannot resolve stale plugin validator conflict #"
            . (int)($conflict['id'] ?? 0)
            . ($replacement === '' ? '' : "; replacement conflict$replacement exists")
            . '; rerun plugin audit and resolve the current validator finding');
    }
    if ($status === 'error') {
        throw new RuntimeException("$command cannot prove plugin validator conflict #"
            . (int)($conflict['id'] ?? 0)
            . ' is current: '
...
function cow_merge_write_plugin_driver_context_file(string $metadata_db, array $context): string {
    $context['metadata_db'] = $metadata_db;
    $dir = cow_merge_validator_context_dir($metadata_db, (int)$context['run_id']) . DIRECTORY_SEPARATOR . 'driver-conflicts';
    cow_merge_mkdir_p($dir);
    $path = $dir . DIRECTORY_SEPARATOR . 'conflict-' . (int)$context['conflict_id'] . '.json';
    $encoded = json_encode($context, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
    if (!is_string($encoded)) {
        throw new RuntimeException('failed to encode plugin driver conflict context');
    }
    $tmp = $path . '.tmp.' . getmypid();
    if (file_put_contents($tmp, $encoded . "\n") === false) {
        throw new RuntimeException("failed to write plugin driver conflict context: $path");
    }
    if (!@rename($tmp, $path)) {
        @unlink($tmp);
        throw new RuntimeException("failed to publish plugin driver conflict context: $path");
    }
    return $path;
}
...
function cow_merge_run_plugin_driver(
    ...
    try {
        cow_merge_ensure_metadata($meta);
        $context = cow_merge_plugin_driver_conflict_context($meta, $conflict_id);
    } finally {
        $meta->close();
    }
    $context_file = cow_merge_write_plugin_driver_context_file($metadata_db, $context);
    $merge_context = $context['merge'];
    ...
    $target_snapshot = $target_db !== '' && is_file($target_db)
        ...
        : null;
    $filesystem_snapshot = $target_root !== '' && is_dir($target_root)
    ...
    $env = array_merge($_ENV, [
        ...
        'FORKPRESS_MERGE_CONFLICT_ID' => (string)$context['conflict_id'],
        'FORKPRESS_MERGE_CONFLICT_KEY' => (string)($context['conflict_key'] ?? ''),
        'FORKPRESS_MERGE_CONFLICT_TYPE' => (string)$context['conflict_type'],
        'FORKPRESS_MERGE_CONFLICT_JSON' => $context_file,
        'FORKPRESS_MERGE_PLUGIN' => (string)($plugin_context['plugin'] ?? ''),
        'FORKPRESS_MERGE_PLUGIN_OBJECT' => (string)($plugin_context['object'] ?? ''),
        'FORKPRESS_MERGE_SOURCE_BRANCH' => (string)$merge_context['source_branch'],
        'FORKPRESS_MERGE_TARGET_BRANCH' => (string)$merge_context['target_branch'],
        'FORKPRESS_MERGE_BASE_DB' => (string)$merge_context['base_db'],
        'FORKPRESS_MERGE_SOURCE_DB' => (string)$merge_context['source_db'],
        'FORKPRESS_MERGE_TARGET_DB' => (string)$merge_context['target_db'],
        'FORKPRESS_MERGE_TARGET_BEFORE_DB' => (string)$merge_context['target_before_db'],
        'FORKPRESS_MERGE_BASE_ROOT' => (string)$merge_context['base_root'],
        'FORKPRESS_MERGE_SOURCE_ROOT' => (string)$merge_context['source_root'],
        'FORKPRESS_MERGE_TARGET_ROOT' => (string)$merge_context['target_root'],
        'FORKPRESS_MERGE_TARGET_BEFORE_ROOT' => (string)$merge_context['target_before_root'],
    ]);
    try {
        $shell_command = implode(' ', array_map('escapeshellarg', $command));
        $pipes = [];
        ...
        if (!is_resource($process)) {
            throw new RuntimeException("failed to start plugin driver $driver");
        }
        fclose($pipes[0]);
        $stdout = stream_get_contents($pipes[1]);
        $stderr = stream_get_contents($pipes[2]);
        ...
        if ($exit_status !== 0) {
            $stderr = trim(is_string($stderr) ? $stderr : '');
            throw new RuntimeException("plugin driver $driver exited with status $exit_status" . ($stderr === '' ? '' : ": $stderr"));
        }
        $decoded = cow_merge_decode_plugin_driver_stdout(is_string($stdout) ? $stdout : '', $driver);
        if (
            !(bool)$decoded['applied']
            && (
                cow_merge_sqlite_snapshot_changed($target_snapshot)
                || cow_merge_file_root_snapshot_changed($filesystem_snapshot, $target_root)
            )
        ) {
        ...
        $resolution = cow_merge_record_plugin_driver_resolution(
            ...
            $decoded['previous'],
            (bool)$decoded['applied'],
            $note ?? (string)($decoded['note'] ?? 'plugin driver result recorded'),
            $reviewer
        );
        ...
        if ($rollback_errors !== []) {
            ...
            $rollback_failure = implode('; ', array_map(fn(Throwable $error): string => cow_merge_failure_reason($error), $rollback_errors));
            $rollback_artifacts = [
            ...
            cow_merge_record_rollback_failure_artifact(
                $metadata_db,
                (int)$context['run_id'],
                (string)$merge_context['source_branch'],
                (string)$merge_context['target_branch'],
                (string)$merge_context['base_db'],
                (string)$merge_context['source_db'],
                $target_db,
            ...
            throw new CowMergeRollbackFailureException(
                $e->getMessage() . '; plugin driver rollback failed: ' . $rollback_failure,
                cow_merge_failure_reason($e),
                $rollback_failure,
                $rollback_artifacts,
                $e
...
function cow_merge_audit_resolution_choice_filter(?string $value, string $label): ?string {
    ...
    $choices = $label === 'blocked-resolution-choice'
        ? ['source', 'target']
        : ['source', 'target', 'plugin-driver'];
    if (!in_array($value, $choices, true)) {
        $expected = $label === 'blocked-resolution-choice' ? 'source or target' : 'source, target, or plugin-driver';
        throw new InvalidArgumentException("--$label must be $expected");
    }
    return $value;
}
...
function cow_merge_audit_group_by(?string $value): string {
    $group_by = $value ?? 'none';
    if (!in_array($group_by, ['none', 'table', 'status', 'path', 'type', 'severity', 'lifecycle', 'event-type', 'next-action', 'conflict-key', 'resolution-strategy', 'generic-resolver', 'after-revalidate', 'revalidation-class', 'latest-revalidation-status', 'stale-status', 'plugin', 'plugin-object', 'plugin-severity', 'plugin-logical-identity', 'plugin-file'], true)) {
        throw new InvalidArgumentException('--group-by must be none, table, status, path, type, severity, lifecycle, event-type, next-action, conflict-key, resolution-strategy, generic-resolver, after-revalidate, revalidation-class, latest-revalidation-status, stale-status, plugin, plugin-object, plugin-severity, plugin-logical-identity, or plugin-file');
    }
    return $group_by;
}
...
function cow_merge_audit_semantic_scope_filter(?string $value): ?string {
    if ($value === null || $value === '') {
        return null;
    }
    if (!in_array($value, ['wordpress', 'plugin'], true)) {
        throw new InvalidArgumentException('--semantic-scope must be wordpress or plugin');
    }
    return $value;
}

function cow_merge_audit_plugin_file_filter(?string $value): ?string {
    return cow_merge_audit_filter_text($value, 'plugin-file');
}
...
function cow_merge_audit_apply_shortcuts(array $filters): array {
    ...
    $plugin_filter = (($filters['plugin'] ?? null) !== null && (string)$filters['plugin'] !== '') ||
        ...
        (($filters['semantic_scope'] ?? null) !== null && (string)$filters['semantic_scope'] !== '') ||
        (($filters['plugin_file'] ?? null) !== null && (string)$filters['plugin_file'] !== '');
    ...
    if ($resolution_choice) {
        if (cow_merge_audit_filter_is_default_all($filters, 'records')) {
            $filters['records'] = ($filters['resolution_choice'] ?? null) === 'plugin-driver' ? 'resolutions' : 'conflicts';
        } elseif (!in_array(($filters['records'] ?? null), ['conflicts', 'conflict-events', 'resolutions'], true)) {
            throw new InvalidArgumentException('--resolution-choice can only be combined with --records conflicts, conflict-events, or resolutions');
        }
        if (($filters['decision'] ?? null) !== null) {
            throw new InvalidArgumentException('--resolution-choice cannot be combined with --decision');
        }
    }
    if ($blocked_resolution_choice) {
        if (cow_merge_audit_filter_is_default_all($filters, 'records')) {
            $filters['records'] = 'conflicts';
        } elseif (!in_array(($filters['records'] ?? null), ['conflicts', 'conflict-events'], true)) {
            throw new InvalidArgumentException('--blocked-resolution-choice can only be combined with --records conflicts or conflict-events');
        }
        if (($filters['decision'] ?? null) !== null) {
            throw new InvalidArgumentException('--blocked-resolution-choice cannot be combined with --decision');
        }
        if ($resolution_status) {
            throw new InvalidArgumentException('--blocked-resolution-choice cannot be combined with --resolution-status');
        }
    }
    ...
    if ($plugin_filter) {
        if (($filters['scope'] ?? null) === null || ($filters['scope'] ?? null) === '' || ($filters['scope'] ?? null) === 'all') {
            ...
            throw new InvalidArgumentException('--plugin, --plugin-object, --plugin-severity, --plugin-logical-identity, --semantic-scope, and --plugin-file require plugin audit scope');
        }
        if (cow_merge_audit_filter_is_default_all($filters, 'records')) {
            $filters['records'] = $resolution_status ? 'resolutions' : 'conflicts';
        } elseif (!in_array(($filters['records'] ?? null), ['conflicts', 'conflict-events', 'resolutions'], true)) {
            throw new InvalidArgumentException('--plugin, --plugin-object, --plugin-severity, --plugin-logical-identity, --semantic-scope, and --plugin-file can only be combined with --records conflicts, conflict-events, or resolutions');
        }
        if (($filters['decision'] ?? null) !== null) {
            throw new InvalidArgumentException('--plugin, --plugin-object, --plugin-severity, --plugin-logical-identity, --semantic-scope, and --plugin-file cannot be combined with --decision');
        }
        if ($id_band_skips) {
            throw new InvalidArgumentException('--plugin, --plugin-object, --plugin-severity, --plugin-logical-identity, --semantic-scope, and --plugin-file cannot be combined with --id-band-skips');
        }
        if ($target_kept) {
            throw new InvalidArgumentException('--plugin, --plugin-object, --plugin-severity, --plugin-logical-identity, --semantic-scope, and --plugin-file cannot be combined with --target-kept');
        }
    }
    ...
    if ($group_by !== '' && $group_by !== 'none') {
        if (cow_merge_audit_filter_is_default_all($filters, 'records')) {
            $filters['records'] = $group_by === 'event-type'
                ? 'conflict-events'
                : (in_array($group_by, ['lifecycle', 'next-action', 'conflict-key', 'resolution-strategy', 'generic-resolver', 'after-revalidate', 'revalidation-class', 'latest-revalidation-status', 'stale-status', 'plugin', 'plugin-object', 'plugin-severity', 'plugin-logical-identity', 'plugin-file'], true) ? 'conflicts' : 'resolutions');
        } elseif (!in_array(($filters['records'] ?? null), ['conflicts', 'conflict-events', 'decisions', 'resolutions'], true)) {
        ...
        if ($records === 'resolutions' && !in_array($group_by, ['table', 'status', 'path', 'plugin', 'plugin-object', 'plugin-severity', 'plugin-logical-identity', 'plugin-file'], true)) {
            throw new InvalidArgumentException('--records resolutions supports --group-by table, status, path, plugin, plugin-object, plugin-severity, plugin-logical-identity, or plugin-file');
        }
        if ($records === 'conflicts' && !in_array($group_by, ['table', 'type', 'path', 'severity', 'lifecycle', 'next-action', 'conflict-key', 'resolution-strategy', 'generic-resolver', 'after-revalidate', 'revalidation-class', 'latest-revalidation-status', 'stale-status', 'plugin', 'plugin-object', 'plugin-severity', 'plugin-logical-identity', 'plugin-file'], true)) {
        ...
        if ($records === 'conflict-events' && !in_array($group_by, ['table', 'type', 'lifecycle', 'event-type', 'conflict-key', 'plugin', 'plugin-object', 'plugin-severity', 'plugin-logical-identity', 'plugin-file'], true)) {
            throw new InvalidArgumentException('--records conflict-events supports --group-by table, type, lifecycle, event-type, conflict-key, plugin, plugin-object, plugin-severity, plugin-logical-identity, or plugin-file');
        }
        ...
        if (($filters['conflict_type'] ?? null) !== null && $records !== 'conflict-events') {
...
function cow_merge_audit_filters(array $filters = []): array {
    ...
    if (in_array($records, ['rollback-failures', 'crash-recovery'], true)) {
        ...
        foreach (['conflict_type', 'conflict_id', 'conflict_key', 'event_type', 'plugin', 'plugin_object', 'plugin_severity', 'plugin_logical_identity', 'semantic_scope', 'plugin_file', 'decision', 'review_status', 'resolution_status', 'lifecycle_state', 'next_action', 'revalidation_class', 'latest_revalidation_status', 'stale_status', 'resolution_choice', 'blocked_resolution_choice', 'resolution_strategy', 'generic_resolver', 'after_revalidate'] as $key) {
    ...
    return [
        ...
        'semantic_scope' => cow_merge_audit_semantic_scope_filter($filters['semantic_scope'] ?? null),
        'plugin_file' => cow_merge_audit_plugin_file_filter($filters['plugin_file'] ?? null),
...
function cow_merge_audit_plugin_semantic_scope_group(?string $payload_json): ?string {
    $scope = cow_merge_audit_plugin_payload_field($payload_json, 'semantic_scope');
    return in_array($scope, ['wordpress', 'plugin'], true) ? $scope : null;
}

function cow_merge_audit_plugin_logical_identity_group(?string $payload_json): ?string {
    if ($payload_json === null || $payload_json === '') {
        return null;
    }
    $decoded = json_decode($payload_json, true);
...
function cow_merge_audit_plugin_file_matches(?string $payload_json, ?string $file): int {
    if ($payload_json === null || $payload_json === '' || $file === null || $file === '') {
        return 0;
    }
    $decoded = json_decode($payload_json, true);
    if (!is_array($decoded)) {
    ...
    foreach ($payload['files'] as $candidate) {
        if (is_string($candidate) && hash_equals($file, $candidate)) {
            return 1;
        }
    }
    return 0;
...
function cow_merge_audit_plugin_file_group(?string $payload_json): ?string {
    if ($payload_json === null || $payload_json === '') {
        return null;
    }
    $decoded = json_decode($payload_json, true);
...
function cow_merge_audit_conflict_next_action(
    ...
    $row = [
        ...
        'latest_event_type' => is_string($latest_event_type) ? $latest_event_type : '',
    ];
...
function cow_merge_audit_conflict_contract_field(
    mixed $table_name,
    mixed $conflict_type,
    mixed $row_identity,
    mixed $column_name,
    ...
    if ($field === 'resolution-strategy') {
        return (string)$contract['strategy'];
    }
    if ($field === 'generic-resolver') {
        return !empty($contract['generic_resolver']) ? 'yes' : 'no';
    }
    if ($field === 'after-revalidate') {
        return !empty($contract['after_revalidate']) ? 'supported' : 'unsupported';
    }
    return null;
}
...
function cow_merge_audit_register_functions(SQLite3 $db): void {
    ...
    if (!$db->createFunction(
        'forkpress_plugin_logical_identity_group',
        fn($payload_json) => cow_merge_audit_plugin_logical_identity_group(is_string($payload_json) ? $payload_json : null),
        1
    )) {
        throw new RuntimeException('failed to register audit plugin logical identity group function');
    }
    if (!$db->createFunction(
        'forkpress_plugin_semantic_scope_group',
        fn($payload_json) => cow_merge_audit_plugin_semantic_scope_group(is_string($payload_json) ? $payload_json : null),
        1
    )) {
        throw new RuntimeException('failed to register audit plugin semantic scope group function');
    }
    if (!$db->createFunction(
        'forkpress_plugin_file_matches',
        fn($payload_json, $file) => cow_merge_audit_plugin_file_matches(
            is_string($payload_json) ? $payload_json : null,
            is_string($file) ? $file : null
        ),
        2
    )) {
        throw new RuntimeException('failed to register audit plugin file filter function');
    }
    if (!$db->createFunction(
        'forkpress_plugin_file_group',
        fn($payload_json) => cow_merge_audit_plugin_file_group(is_string($payload_json) ? $payload_json : null),
        1
    )) {
        throw new RuntimeException('failed to register audit plugin file group function');
    }
    if (!$db->createFunction(
...
function cow_merge_audit_conflict_lifecycle_state_sql(
    ...
    $latest_review_status = $review_notes_exist
        ...
        : 'NULL';
    $latest_event_type = $conflict_events_exist
        ? "(SELECT ce.event_type FROM merge_conflict_events ce WHERE ce.conflict_id = $conflict_id_sql ORDER BY ce.id DESC LIMIT 1)"
        : 'NULL';
...
function cow_merge_audit_conflict_next_action_sql(
    ...
    $latest_resolution_applied = $resolutions_exist
        ...
        : '0';
    $latest_event_type = $conflict_events_exist
        ? "COALESCE((SELECT ce.event_type FROM merge_conflict_events ce WHERE ce.conflict_id = " . $record . ".id ORDER BY ce.id DESC LIMIT 1), '')"
        : "''";
...
function cow_merge_audit_conflict_contract_field_sql(string $alias, string $field): string {
    $record = $alias === '' ? 'merge_conflicts' : $alias;
    $field_sql = SQLite3::escapeString($field);
    return 'forkpress_conflict_contract_field(' .
        $record . '.table_name, ' .
        $record . '.conflict_type, ' .
        $record . '.row_identity, ' .
        $record . '.column_name, ' .
...
function cow_merge_audit_where_sql(
    ...
    if ($record_type === 'conflicts' && ($filters['semantic_scope'] ?? null) !== null) {
        $clauses[] = $prefix . "table_name = '__plugins__'";
        $clauses[] = 'forkpress_plugin_semantic_scope_group(' . $prefix . 'chosen_payload) = :semantic_scope_filter';
        $params[':semantic_scope_filter'] = $filters['semantic_scope'];
    }

    if ($record_type === 'conflicts' && ($filters['plugin_file'] ?? null) !== null) {
        $clauses[] = $prefix . "table_name = '__plugins__'";
        $clauses[] = 'forkpress_plugin_file_matches(' . $prefix . 'chosen_payload, :plugin_file_filter) = 1';
        $params[':plugin_file_filter'] = $filters['plugin_file'];
    }
    ...
    if ($record_type === 'conflicts' && ($filters['next_action'] ?? null) !== null) {
        $clauses[] = cow_merge_audit_conflict_next_action_sql($alias, $review_notes_exist, $resolutions_exist, $conflict_events_exist) . ' = :next_action';
...
function cow_merge_audit_resolution_where_sql(
    ...
    if (($filters['resolution_choice'] ?? null) !== null) {
        $clauses[] = 'mr.choice = :resolution_choice';
        $params[':resolution_choice'] = $filters['resolution_choice'];
    }
    ...
    if (($filters['semantic_scope'] ?? null) !== null) {
        $clauses[] = "c.table_name = '__plugins__'";
        $clauses[] = 'forkpress_plugin_semantic_scope_group(c.chosen_payload) = :semantic_scope_filter';
        $params[':semantic_scope_filter'] = $filters['semantic_scope'];
    }

    if (($filters['plugin_file'] ?? null) !== null) {
        $clauses[] = "c.table_name = '__plugins__'";
        $clauses[] = 'forkpress_plugin_file_matches(c.chosen_payload, :plugin_file_filter) = 1';
        $params[':plugin_file_filter'] = $filters['plugin_file'];
    }
...
function cow_merge_audit_resolution_group_sql(string $group_by): string {
    ...
    if ($group_by === 'plugin-file') {
        return "CASE WHEN mr.table_name = '__plugins__' THEN COALESCE(forkpress_plugin_file_group(c.chosen_payload), '(unknown)') ELSE mr.table_name END";
    }
...
function cow_merge_audit_conflict_group_sql(
    ...
    if ($group_by === 'lifecycle') {
        return cow_merge_audit_conflict_lifecycle_state_sql('c.id', $review_notes_exist, $resolutions_exist, $conflict_events_exist);
    }
    if ($group_by === 'next-action') {
        return cow_merge_audit_conflict_next_action_sql('c', $review_notes_exist, $resolutions_exist, $conflict_events_exist);
    }
    if ($group_by === 'conflict-key') {
        return $conflict_key_exists ? "COALESCE(NULLIF(c.conflict_key, ''), '(none)')" : "'(none)'";
    }
    if (in_array($group_by, ['resolution-strategy', 'generic-resolver', 'after-revalidate'], true)) {
        return "COALESCE(" . cow_merge_audit_conflict_contract_field_sql('c', $group_by) . ", 'unknown')";
    }
    ...
    if ($group_by === 'plugin-file') {
        return "CASE WHEN c.table_name = '__plugins__' THEN COALESCE(forkpress_plugin_file_group(c.chosen_payload), '(unknown)') ELSE c.table_name END";
    }
    throw new InvalidArgumentException('unsupported conflict group');
}

function cow_merge_audit_conflict_event_group_sql(string $group_by, bool $conflict_key_exists = true): string {
    if ($group_by === 'table') {
        return 'c.table_name';
    }
    if ($group_by === 'type') {
    ...
    if ($group_by === 'lifecycle') {
        return 'ce.lifecycle_state';
    }
    if ($group_by === 'event-type') {
        return 'ce.event_type';
    }
    if ($group_by === 'conflict-key') {
        return $conflict_key_exists ? "COALESCE(NULLIF(c.conflict_key, ''), '(none)')" : "'(none)'";
    }
...
function cow_merge_audit_count_sql(
    ...
    if ($record_type === 'conflicts' && ($filters['semantic_scope'] ?? null) !== null) {
        $conditions[] = $alias . ".table_name = '__plugins__'";
        $conditions[] = "forkpress_plugin_semantic_scope_group($alias.chosen_payload) = '" . SQLite3::escapeString($filters['semantic_scope']) . "'";
    }
    if ($record_type === 'conflicts' && ($filters['plugin_file'] ?? null) !== null) {
        $conditions[] = $alias . ".table_name = '__plugins__'";
        $conditions[] = "forkpress_plugin_file_matches($alias.chosen_payload, '" . SQLite3::escapeString($filters['plugin_file']) . "') = 1";
    }
    if ($record_type === 'conflicts' && ($filters['lifecycle_state'] ?? null) !== null && ($filters['records'] ?? null) !== 'conflict-events') {
    ...
    if ($record_type === 'conflicts' && ($filters['next_action'] ?? null) !== null) {
        $conditions[] = cow_merge_audit_conflict_next_action_sql($alias, $review_notes_exist, $resolutions_exist, $conflict_events_exist) .
...
function cow_merge_conflict_lifecycle(array $row): array {
    ...
    $latest_event_type = (string)($row['latest_event_type'] ?? '');
...
function cow_merge_audit_report(string $metadata_db, ?int $run_id = null, int $limit = 20, array $filters = []): array {
    ...
    try {
        ...
        $decision_count_filter = cow_merge_audit_count_sql($filters, 'decisions', 'd', $review_notes_exist, $resolutions_exist, $revalidations_exist, $conflict_events_exist);
        $conflict_count_filter = cow_merge_audit_count_sql($filters, 'conflicts', 'c', $review_notes_exist, $resolutions_exist, $revalidations_exist, $conflict_events_exist);
        ...
        $id_band_filter = cow_merge_audit_count_sql($filters, 'decisions', 'd', $review_notes_exist, $resolutions_exist, $revalidations_exist, $conflict_events_exist);
        ...
        if ($filters['records'] === 'all' || $filters['records'] === 'conflicts') {
            ...
            if ($filters['records'] === 'conflicts' && $filters['group_by'] !== 'none') {
                [$conflict_group_filter, $conflict_group_params] = cow_merge_audit_where_sql($run_id, $filters, 'conflicts', 'c', $review_notes_exist, $resolutions_exist, $revalidations_exist, $conflict_events_exist);
                $conflict_group_params[':limit'] = $limit;
                $group_expr = cow_merge_audit_conflict_group_sql($filters['group_by'], $review_notes_exist, $resolutions_exist, $conflict_key_exists, $revalidations_exist, $conflict_events_exist);
        ...
        if ($filters['records'] === 'all' || $filters['records'] === 'conflict-events') {
            ...
            [$event_filter, $event_params] = cow_merge_audit_where_sql($run_id, $event_filter_source, 'conflicts', 'c', $review_notes_exist, $resolutions_exist, $revalidations_exist, $conflict_events_exist);
            ...
            if ($filters['records'] === 'conflict-events' && $filters['group_by'] !== 'none') {
                ...
                $report['conflict_event_groups'] = cow_merge_audit_table_rows(
                    $db,
                    'merge_conflict_events',
                    "SELECT :group_by AS group_by, $group_expr AS group_key, COUNT(*) AS event_count, " .
                    "COUNT(DISTINCT ce.conflict_id) AS conflict_count, " .
                    "SUM(CASE WHEN c.table_name = '__files__' THEN 1 ELSE 0 END) AS file_count, " .
                    "SUM(CASE WHEN c.table_name = '__plugins__' THEN 1 ELSE 0 END) AS plugin_count, " .
                    "SUM(CASE WHEN c.table_name NOT IN ('__files__', '__plugins__') THEN 1 ELSE 0 END) AS db_count " .
                    "FROM merge_conflict_events ce " .
                    "JOIN merge_conflicts c ON c.id = ce.conflict_id " .
                    "JOIN merge_runs r ON r.id = ce.run_id " .
                    "$event_filter GROUP BY group_key ORDER BY event_count DESC, group_key LIMIT :limit",
                    $event_params + [':group_by' => $filters['group_by']]
                );
            }
        }

        [$decision_filter, $decision_params] = cow_merge_audit_where_sql($run_id, $filters, 'decisions', '', $review_notes_exist, $resolutions_exist, $revalidations_exist, $conflict_events_exist);
        ...
        if ($filters['records'] === 'all' || $filters['records'] === 'decisions') {
            ...
            if ($filters['records'] === 'decisions' && $filters['group_by'] !== 'none') {
                [$decision_group_filter, $decision_group_params] = cow_merge_audit_where_sql($run_id, $filters, 'decisions', 'd', $review_notes_exist, $resolutions_exist, $revalidations_exist, $conflict_events_exist);
...
function cow_merge_print_audit_text(array $report): void {
    ...
    if ($report['conflict_event_groups']) {
        echo "conflict-event-groups:\n";
        foreach ($report['conflict_event_groups'] as $group) {
            echo "  {$group['group_by']}={$group['group_key']} events={$group['event_count']} conflicts={$group['conflict_count']} files={$group['file_count']} plugin={$group['plugin_count']} db={$group['db_count']}\n";
        }
    }
...
if (realpath($argv[0] ?? '') === __FILE__) {
    try {
        ...
        if ($command === 'run-plugin-driver') {
            $args = cow_merge_parse_cli($argv, ['metadata-db', 'driver'], 2);
            $has_id = array_key_exists('id', $args) && (string)$args['id'] !== '';
            $has_conflict_key = array_key_exists('conflict-key', $args) && (string)$args['conflict-key'] !== '';
            if ($has_id === $has_conflict_key) {
                throw new InvalidArgumentException('run-plugin-driver requires exactly one of --id or --conflict-key');
            }
            $run_id = array_key_exists('run', $args)
                ? cow_merge_audit_run_id($args['run'] ?? null)
                : null;
            ...
            $result = cow_merge_run_plugin_driver(
                $args['metadata-db'],
                $conflict_id,
                $args['driver'],
                array_key_exists('note', $args) ? cow_merge_review_text($args['note'], 'note') : null,
                cow_merge_review_text($args['reviewer'] ?? 'plugin-driver', 'reviewer')
            );
            $format = cow_merge_audit_format($args['format'] ?? null);
            if ($format === 'json') {
                $encoded = json_encode($result, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
                if (!is_string($encoded)) {
                    throw new RuntimeException('failed to encode plugin driver run result');
                }
                echo $encoded . "\n";
            } elseif (($args['quiet'] ?? '0') !== '1') {
                echo "forkpress: ran plugin driver\n";
                echo "  conflict:  #{$result['conflict_id']}\n";
                echo "  resolution:#{$result['resolution_id']}\n";
                echo "  status:    {$result['driver_status']}\n";
                echo "  driver:    {$result['driver']}\n";
                echo "  applied:   " . ($result['applied'] ? 'yes' : 'no') . "\n";
                echo "  metadata:  {$result['metadata_db']}\n";
            }
        ...
        if ($command === 'audit') {
            ...
            $report = cow_merge_audit_report(
                ...
                [
                    ...
                    'semantic_scope' => $args['semantic-scope'] ?? null,
                    'plugin_file' => $args['plugin-file'] ?? null,
